...
1. Bootstrap信頼区間を出すときには何回繰り返しが必要でしょうか？
1. Bootstrap信頼区間を出す前に必ずやらないといけない方法はなんでしょうか？
1. Bootstrap信頼区間の時に良い方法はなんでしょうか？
...
-   代表的なものは以下の4つ
    1.  標本から計算した統計量にBootstrap標本から計算した分散を用いた正規分布を足し算して計算する(Normal法) <br> 
→ sd = `r sd(boot_height$t)`
    1.  2.5% \~ 97.5%の範囲をそのまま提示する(Percentile法) <br>
→ [`r quantile(boot_height$t, probs = c(0.025, 0.0975))`]
    1.  標本から計算した統計量にBootstrap標本の2.5%tile, 97.5%tileの値を引き算する(Basic法)
    1.  Percentile法を補正したBCa法
    1.  事前に分かっている分散を用いて、統計量を割り算してt分布で近似する(Bootstrap-t法)
::: callout-note
...
-   分散が分かればBootstrap-tが良いらしいが、分散が分かっているなら最初からBootstrapを使わない気もする
-   BCa法はBootstrapの繰り返しが多い必要がある
...
## Bootstrapの素朴な疑問 1
-   nは幾つにすればいいの？<br>
→　Efron曰く、分散だけなら200程度、信頼区間なら1000-2000程度必要

## Bootstrapの素朴な疑問 2
-   元データは少なくてもいいの？
→　少なすぎるとBiasは補正しきれない。大元のデータがちゃんと(母集団を)近似するくらいじゃないとだめ
-   ロジスティック回帰やCox回帰などで、Bootstrapでエラーが出る(偶然、Bootstrap標本でイベントが一回も起きない時)があるが、その際はおそらく大元のデータが足りない

## クイズの答え

![](images/bootstrap_answer.jpeg){height="300px" fig-align="center"} 

1. Bootstrapは靴の中のどの部分でしょうか？
1. Bootstrap信頼区間を出すときには1000-2000回繰り返しをする
1. Bootstrap信頼区間を出す前に必ず**統計量のHistogram**を確認する！
1. Bootstrap信頼区間の時には**BCa法を行う！
